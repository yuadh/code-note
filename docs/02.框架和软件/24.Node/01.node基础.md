...
path模块是 Node 官方提供的、用来处理路径的模块。它提供了一系列的方法和属性，用来满足用户对路径的处理需求
...
使用path.join()方法，可以把多个路径片段拼接为完整的路径
...
http模块是 Node.js 官方提供的、用来创建web服务器的模块。通过http模块提供的 `http.createServer()`方法，就能方便的把一台普通的电脑，变成一台web服务器

### 服务器相关概念

- IP地址(a.b.c.d)   localhost（本地服务器）
- 域名和域名服务器（提供IP地址和域名之间的转换）
- 端口号 （对应web服务）80端口可以被省略
...
服务器接受到了客户端的请求，就会调用通过 server.on() 为服务器绑定request事件处理函数
